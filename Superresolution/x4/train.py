...
if __name__ == '__main__':
	...
	for epoch in range(start_epoch, opt.end_epoch + 1):
		...
		for i_batch, sample_batched in enumerate(dataloader):
			...
			print('\r Epoch : ' + str(epoch) + ' | (' + str(i_batch+1) + '/' + str(batches) + ') | l1: ' + str(opt.batch_mse_loss) + ' | vgg: ' + str(opt.batch_vgg_loss) + ' | ssim: ' + str(opt.batch_ssim_loss), end='', flush=True)
			# bar.next()
			
 

		print('\nFinished ep. %d, lr = %.6f, total_l1 = %.6f, total_vgg = %.6f, total_ssim = %.6f' % (epoch, get_lr(optim_g), opt.total_mse_loss, opt.total_vgg_loss, opt.total_ssim_loss))
